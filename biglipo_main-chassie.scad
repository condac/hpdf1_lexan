...
translate([24,11,5]) receiver();
translate([-24,11,5]) rotate([0,-90]) ESC_big();
...
bp_z =1.5;
body_post_x = 71;
body_post_y = 52-pp1_y + long_extra;
body_post_z = 50-pp2_z;
body_post_z_legacy = 45-pp2_z;

module biglipo_main_chassie_p() {
    difference() {
        union() {
            translate([0,0,0]) nose_p();
            translate([0, 0, p_offset_z]) plattan();
            translate([0, 0, p_offset_z]) mirror([1,0,0])plattan();
            translate([-b_x/2, 0, 0]) cube([b_x, batteri_y+bakre_wall+2, b_z]); // biglipo ändrat
            translate([-batteri_y2/2-1, bakre_wall+1, 0]) cube([batteri_y2+2, batteri_x+4, b_z]);
            ...
            //for (a =[0:4:batteri_y2]) {
             //   translate([-batteri_y2/2 + a,10,0.5]) cube([2,batteri_x, 1.5]);
            //}
            ...
            hull() {
                ...
                translate([batteri_y2/2,bakre_wall+1,b_z]) cube([1,3,1]);
                translate([batteri_x/2,bakre_wall+1,b_z]) cube([1,3,1]);
            }
            hull() {
                translate([-pp1_x, bakre_wall+1, pp1_z-1]) cube([1,3,1]);
                translate([-batteri_y2/2-1,bakre_wall+1,b_z]) cube([1,3,1]);
        ...
        //translate([batteri_y2/2, bakre_wall+4, b_offset_z]) rotate([0,0,90]) batteri();
        translate([-batteri_x/2, bakre_wall, 2]) rotate([0,0,0]) batteri();
        
        
        // för att kunna ta ut batteriet på sidan
        translate([batteri_y2/2-2, bakre_wall+4, b_offset_z+3]) rotate([0,0,90]) batteri();
        translate([batteri_y2/2+2, bakre_wall+4, b_offset_z+3]) rotate([0,0,90]) batteri();
...
module radio_tub() {
    ...
    yy = 35;
    wall = 0.4*3;
    translate([0,batteri_x+bakre_wall+10+long_extra,0]) difference() {
...
module radio_tub_cut() {
    ...
    translate([0,batteri_x+bakre_wall+11+long_extra,0]) translate([-xx/2+wall, wall, 2]) cube([xx-wall*2,yy-wall*2,b_z]);
...
module biglipo_battery_plate_p() {
    translate([0,0,pp2_z]) difference() {
        ...
        esc_cut();
        mirror([1,0,0])esc_cut();
    }
}

module esc_cut() {
    translate([24,11,0]) cube([20,47,9.5]);
    translate([24,11+47,0+9.5]) rotate([90+45,0,0]) cube([20,47,9.5]);
}
...
module nose_plate_p() {
    ...
    translate([0,0,4]) difference() {
        ...
        translate([-15,nose_main_mount_y-nose_main_mount_space,-1]) cylinder(d=m3_d_s, h=4+2);
        translate([15,nose_main_mount_y-nose_main_mount_space,-1]) cylinder(d=m3_d_s, h=4+2);
        translate([-15,nose_main_mount_y+nose_main_mount_space,-1]) cylinder(d=m3_d_s, h=4+2);
        translate([15,nose_main_mount_y+nose_main_mount_space,0-1]) cylinder(d=m3_d_s, h=4+2);
...
module crossplate(xdim1, xdim2, ,ydim ,zdim, rdim){
    difference() {
        ...
        translate([xdim2,ydim,-1]) cylinder(d=m3_d, h=zdim+2);
        translate([-xdim2,ydim,-1]) cylinder(d=m3_d, h=zdim+2);
        translate([xdim1,0,-1]) cylinder(d=m3_d, h=zdim+2);
        translate([-xdim1,0,-1]) cylinder(d=m3_d, h=zdim+2);
    }    
}
module crossplate_bat(xdim1, xdim2, ,ydim ,zdim, rdim, post_z){
    difference() {
        union() {
            ...
            hull() {
                translate([20,0,0]) cylinder(r=rdim, h=zdim);
                translate([20,ydim,0]) cylinder(r=rdim, h=zdim);
            }
            hull() {
                translate([-20,0,0]) cylinder(r=rdim, h=zdim);
                translate([-20,ydim,0]) cylinder(r=rdim, h=zdim);
            ...
            hull() {
                translate([-23,0,0]) cylinder(r=rdim, h=zdim);
                translate([xdim2,ydim,0]) cylinder(r=rdim, h=zdim);
            }
            hull() {
                translate([23,0,0]) cylinder(r=rdim, h=zdim);
            ...
            hull() {
                translate([20,8,0]) cylinder(r=rdim, h=zdim);
                translate([-20,8,0]) cylinder(r=rdim, h=zdim);
            }
        ...
        translate([-96/2+5,4+7,0]) cylinder(d=8, h=20);
        hull() {
            translate([-8, 6,0]) cylinder(d=8, h=5);
            translate([-18, 6,0]) cylinder(d=8, h=5);
        }
        hull() {
            translate([8, 6,0]) cylinder(d=8, h=5);
            translate([18, 6,0]) cylinder(d=8, h=5);
